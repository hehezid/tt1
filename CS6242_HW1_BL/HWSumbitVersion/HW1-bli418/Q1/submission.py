...
def return_argo_lite_snapshot()->str:
    ...
    #url = 'https://poloclub.github.io/argo-graph-lite/#58469ea9-b8e9-44c5-8b61-9a5d8e7072db' #1070 new
    url = 'https://poloclub.github.io/argo-graph-lite/#dab54260-f34d-44ed-a106-6fec70d4f2ef'
    return url
    ...
    def new_node_step1(person_id: int, vote_avg: float, limit: int, exclude_ids: list):
        ...
        if dat1 != []:
            for i in dat1:
                ...
                co_actor1 = tmdb_api_utils.get_movie_cast(movie_id=i['id'], limit=limit, exclude_ids=exclude_ids)
                if co_actor1 !=[]:
                    for item in co_actor1:
                        if item['id'] not in new_id:
                            graph.add_node(id=str(item['id']), name=item['name'])
                            graph.add_edge(source=str(person_id), target=str(item['id']))
                            new_id.append(item['id'])
        return new_id

    new_id=new_node_step1(5064, 8.0, 3, [5064])
    # 2 iteration 3 times
    new = []
    for item in new_id:
        new +=new_node_step1(person_id=item,vote_avg=8.0,limit=3,exclude_ids=[item])

    new2=[]
    for item2 in new:
        new2 +=new_node_step1(person_id=item2,vote_avg=8.0,limit=3,exclude_ids=[item2])
